...
int main(void)
{
    ...
    //Here, the string array can allow a maximum number of 10 strings (10 rows) and each of these 10 strings can hAVE ONLY UPTO 15 CHARCATERS maximum (15 columns)
    char strArray[10][15] = {"Hello!", "Welcome", "To", "Real", "Time", "Rendering", "Batch", "(2020-21)", "Of", "ASTROMEDICOMP."}; //IN-LINE INITIALIZATION
    int iStrLEngth[10]; //1D Interger Array - Stores lengths of those strings at corresponding indices in 'strArray[]' e.g: iStrLengths[0] will be the length of string ata strArray[0], iStrLengths[1] will be the length of string at strArray[1]...10 strings, 10 lengths...
    ...
    // Stroing in lenths of all the strings...
    ...
    // The Column Number (second []) is the particular character is that string / row
