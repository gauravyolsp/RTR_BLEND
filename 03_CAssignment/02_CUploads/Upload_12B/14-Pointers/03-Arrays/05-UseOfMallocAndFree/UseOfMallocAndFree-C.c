...
int main(void)
{
    ...
    int *ptr_iArray = NULL; // IT IS GOOD DISCIPLINE TO INITIALIZE ANY POINTER WITH NULL ADDRESS TO PREVENT ANY GARBAGE VALUE GETTING INTO IT, THAT WAY, IT MAKES IT EASY TO CHECK FOR SUCCESS OR FAILURE OF MEMORY ALLOCATION LATER ON AFTER malloc()...
    ...
    // ****** USING THIS ASE ADDRESS, THE INTEGER ARRAY CAN BE ACCESSED AND USED ******
    ...
    {
        printf("\n\n");
        printf("MEMORY ALLOCATION FOR INTEGER ARRAY HAS SUCCEEDE !!!\n\n");
        printf("MEMORY ADDRESSES FROM %p TO %p HAVE BEEN ALLOCATED TO INTEGER ARRAY !!! \n\n", ptr_iArray, (ptr_iArray + (intArrayLength - 1)));
    }

    else // IF ptr_iaRRAY IS NOT NULL, IT MEANS IT MUST CONTAIN A VALID ADDRESS WHICH IS RETURNED BY malloc(), HWNCE, malloc() HAS SUCCEEDED IN MEMORY ALLOCATION...
    {
        printf("\n\n");
    ...
    // ****** ONCE MEMORY IS FREED USING free(), THE BASE ADDRESS MUST BE CLEANED, THAT IS, IT MUST BE RE-INITILAIZED TO 'NULL' TO KEEP AWAY GARBAGE VALUE. THIS IS NOT COMPULSORY, BUT IT IS GOOD CODING DISCIPLINE ******
    ...
    if(ptr_iArray)
    {
        ...
        printf("\n\n");
        printf("MEMORY ALLOCATED FOR INTGER ARRAY HAS BEEN SUCCESSFULLY FREED !!!\n\n");
    }
