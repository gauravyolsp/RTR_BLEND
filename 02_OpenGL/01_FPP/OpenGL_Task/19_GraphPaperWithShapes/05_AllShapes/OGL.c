...
// My Global variables
float g_XAxis = 0.0f;
BOOL g_bGraphEnable = FALSE;
BOOL g_bCircleEnable = FALSE;
BOOL g_bSquareEnable = FALSE;
BOOL g_bTriangleEnable = FALSE;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(iMsg)
	{
		...
		case WM_CHAR:
			...
			case 'c':
				if (g_bCircleEnable)
					g_bCircleEnable = FALSE;
				else
					g_bCircleEnable = TRUE;
				break;
			case 'G':
			case 'g':
				if (g_bGraphEnable)
					g_bGraphEnable = FALSE;
				else
					g_bGraphEnable = TRUE;
				break;
			case 'S':
			case 's':
				if (g_bSquareEnable)
					g_bSquareEnable = FALSE;
				else
					g_bSquareEnable = TRUE;
				break;
			case 'T':
			case 't':
				if (g_bTriangleEnable)
					g_bTriangleEnable = FALSE;
				else
					g_bTriangleEnable = TRUE;
				break;
			case '0':
				g_bGraphEnable = FALSE;
				g_bCircleEnable = FALSE;
...
{
	// X - Axis
	for (int iCounter = 1; iCounter <= 35; iCounter++)
	{
		g_XAxis = g_XAxis + 0.02f;
	...
	for (int iCounter = 1; iCounter <= 35; iCounter++)
	{
		...
		glBegin(GL_LINES);
		glColor3f(0.0f, 0.0f, 1.0f);
		glVertex3f(-1.0f, g_XAxis, 0.0f);
		glVertex3f(1.0, g_XAxis, 0.0f);
		glEnd();
