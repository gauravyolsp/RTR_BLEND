...
// My Global variables
float g_XAxis = 0.0f;
BOOL g_bCircleWithPoinots = TRUE;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(iMsg)
	{
		...
		case WM_CHAR:
			...
			case 'P':
			case 'p':
				g_bCircleWithPoinots = TRUE;
				break;
			case 'L':
			case 'l':
				g_bCircleWithPoinots = FALSE;
				break;
			}
		break;
		case WM_CLOSE:
...
{
	// Function declarations
	void CircleWithPoints(void);
	void CircleWithLines(void);
...
void CircleWithPoints()
{
	...
	for (float angle = 0.0f; angle < (2 * M_PI); angle += 0.0001)
	{
...
void CircleWithLines()
{
	...
	for (float angle = 0.0f; angle < (2 * M_PI); angle += 0.0001)
	{
		...
		glColor3f(1.0f, 1.0f, 0.0f);
		glVertex3f(x, y, 0.0f);
		glVertex3f(x + 0.002f, y + 0.002f, 0.0f);
		glEnd();
	}
}
