...
// My Global variables
float xTriangle = -0.6f;
float yTriangle = -0.6f;
...
void display(void)
{
	...
	glTranslatef(xTriangle, yTriangle, -1.0f);
	...
	if (g_iStopTriangle == 20 && (xTriangle < 0.0001f && yTriangle < 0.0001f))
	{
		g_iStopTriangle = 0;
	...
	if (bDrawCircle)
	{
		...
		// Translate Triangle Backwards by Z (-ve)
		glTranslatef(xCricle, yCricle, -1.0f);
		...
		if (g_iStopCricle == 20 && (xCricle > 0.0001f && yCricle < 0.0001f))
		{
			g_iStopCricle = 0;
	...
	if (bDrawStick)
	{
		...
		if (g_iStopStick == 20 && yStick > 0.0001f)
		{
			yStick -= 0.001f;
			g_iStopStick = 0;
		}
