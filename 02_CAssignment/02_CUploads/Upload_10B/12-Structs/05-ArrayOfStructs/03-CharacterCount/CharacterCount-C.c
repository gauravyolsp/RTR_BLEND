...
struct CharacterCount
{
    char ch;
    int ch_Count;
} character_and_count[] = {{'A', 0}, // charcter_and_count[0].ch = 'A' & character_and_count[0].ch_count = 0
                            {'B', 0}, //charcter_and_count[1].ch = 'B' & character_and_count[1].ch_count = 0
                            {'C', 0},
                            {'D', 0},
                            {'E', 0},
                            {'F', 0},
...
int main(void)
{
    ...
    for(i = 0; i < actual_string_length; i++)
    {
        ...
        {
            str[i] = toupper(str[i]); // If input charcater is in lower case, turn it to upper case for comparsion
